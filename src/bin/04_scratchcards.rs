...
fn parse_scratchcard(line: &str) -> Scratchcard {
    ...
    let winning: HashSet<u64> = number_chunks[0].split_terminator(" ")
        .map(|nr| nr.trim())
        .filter(|nr| nr.len() > 0)
        .map(|nr| u64::from_str_radix(nr, 10).expect(format!("Parse winning number: {}", nr).as_str())).collect();
    let owned: HashSet<u64> = number_chunks[1].split_terminator(" ")
        .map(|nr| nr.trim())
        .filter(|nr| nr.len() > 0)
        .map(|nr| u64::from_str_radix(nr, 10).expect(format!("Parse owned number: {}", nr).as_str())).collect();
    Scratchcard { winning, owned }
}


fn day_04_scratchcards(input_fpath: &PathBuf) -> (u64, u64) {
    let lines = std::fs::read_to_string(input_fpath).expect(format!("Read input from {:?}", input_fpath).as_str());
    let scratchcards: Vec<Scratchcard> = lines.split_terminator("\n").map(parse_scratchcard).collect();
    let scratch_scores: Vec<usize>  = scratchcards.iter().map(|scratchcard| scratchcard.get_winning_numbers().len())
        .collect();
    let part_one_score = scratch_scores.iter()
...
fn main() {
    println!("{:?}", day_04_scratchcards(&PathBuf::from("input/04-demo.txt")));
